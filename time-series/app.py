...
if st.sidebar.button("Kéo Dữ Liệu Live Từ Yahoo Finance"):
    with st.spinner('Đang tải dữ liệu trực tiếp...'):
        ...
        full_df = pd.concat([buffer_df[['price']], live_df])
        full_df = full_df[~full_df.index.duplicated(keep='last')].sort_index()
else:
    full_df = buffer_df

st.info("💡 Bạn đang xem dữ liệu: **" + ("Buffer Cũ" if len(full_df) == len(buffer_df) else "Live Update Nhất") + "**")
...
if st.button(f"🔥 BẤM DỰ BÁO {forecast_hours} GIỜ TIẾP THEO", type="primary"):
    with st.spinner(f'Đang chạy vòng lặp Autoregressive cho {forecast_hours} giờ...'):
        ...
        if future_fallback_count > 0:
            st.warning(
                f"⚠️ Model trả về giá trị log_return bất thường {future_fallback_count}/{forecast_hours} bước dự báo tương lai. "
