...
# Tự rerun mỗi 1 giờ (không reload browser page)
st_autorefresh(interval=3600 * 1000, key="live_hourly_rerun")


@st.cache_data(ttl=3600)
def fetch_live_hourly_data():
    ...
    # Chuẩn hóa timestamp Yahoo sang giờ Việt Nam (UTC+7)
    if getattr(live_df.index, 'tz', None) is not None:
        live_df.index = live_df.index.tz_convert('Asia/Ho_Chi_Minh').tz_localize(None)
    else:
        live_df.index = live_df.index.tz_localize('UTC').tz_convert('Asia/Ho_Chi_Minh').tz_localize(None)

    return live_df[['price']]


try:
    live_df = fetch_live_hourly_data()
except Exception:
    live_df = pd.DataFrame(columns=['price'])

if len(live_df) > 0:
    merged_df = pd.concat([buffer_df[['price']], live_df])
    full_df = merged_df[~merged_df.index.duplicated(keep='last')].sort_index()
    data_source_label = "Live Auto Update (1h)"
else:
    full_df = buffer_df[['price']].copy()
    data_source_label = "Buffer Cũ (Live tạm thời lỗi)"

st.info("Bạn đang xem dữ liệu: **" + data_source_label + "**")
...
st.caption("Dữ liệu 1h là nến đã đóng gần nhất, có thể trễ vài phút so với thời gian thực.")
